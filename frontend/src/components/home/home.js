...
const Dashboard = () => {
  ...
  const handleAddItem = (data) => {
    axios.post("http://127.0.0.1:8000/api/drivers/", data, {
      headers: { Authorization: `Bearer ${localStorage.getItem("access_token")}` },
    })
    .then((response) => {
  ...
  const handleEditItem = (id, data) => {
    axios.put(`http://127.0.0.1:8000/api/drivers/${id}/`, data, {
      headers: { Authorization: `Bearer ${localStorage.getItem("access_token")}` },
    })
    .then((response) => {
  ...
  useEffect(() => {
    if (selectedDriverId) {
      axios.get(`http://127.0.0.1:8000/api/drivers/${selectedDriverId}/`, {
        headers: { Authorization: `Bearer ${localStorage.getItem("access_token")}` },
      })
      .then((response) => {
